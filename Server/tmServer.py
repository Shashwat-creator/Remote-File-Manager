...
class Model:
    ...
    def populateDatastructure(self,path):
        ...
        with open("users.cfg","r")as usersFile:
            while True:
                ...
                self.users[userName.lower()]=eval(user)
    ...
    def checkUserPassword(self,userName,password):
        dsUser=self.users[userName.lower()]
        dsPassword=dsUser[1]
...
class RequestModifier:
    ...
    def isUserRegistered(socket,model,user):
       # user=request
        ...
        if not(model.checkUserByName(userName)):
            ...
            socket.sendString(str(responseLength).ljust(1024))
            socket.sendString(str(response))
    def __call__(self,socket,model,user):
            RequestModifier.isUserRegistered(socket,model,user);
            self.function(socket,model)


class MyThread(Thread):
    def __init__(self,socket,model,user):
        self.model=model
        self.socket=socket
        self.user=user
        ...
        self.start()
    def run(self):
        acceptRquests(self.socket,self.model,self.user)
